import java.util.function.Supplier;

class Serve implements Event {
    private final Customer customer;
    private final Server server;
    private final double time;

    Serve(Customer cust, Server server, double time) {
        this.customer = cust;
        this.server = server;
        this.time = time;
    }
    
    @Override
    public double getTime() {
        return this.time;
    }

    @Override
    public int getNumber() {
        return this.customer.getNum();
    }

    @Override
    public Server getServer() {
        return this.server;
    }

    @Override
    public Customer getCustomer() {
        return this.customer;
    }

    @Override
    public Stats add(Stats s) {
        return s.add(1, 0, this.time - this.customer.getArriveTime());
    }

    @Override
    public Pair<Server, PQ<Event>> process(ImList<Server> s, PQ<Event> pq,
        Supplier<Double> restTime) {
        int num = Math.min(server.getNum() - 1, s.size() - 1);
        Server server = s.get(num);
        Event e = new Done(customer, server, time + customer.getDuration());
        return new Pair<Server, PQ<Event>>(server, pq.add(e));  
    }

    @Override
    public String toString() {
        return String.format("%.3f %d serves by ", this.getTime(), 
            this.getNumber()) + this.server.toString() + "\n";
    }
}
